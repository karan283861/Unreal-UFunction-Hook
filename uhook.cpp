...
#endif
	processevent_hooks.ExecuteHook(calling_ufunction, calling_uobject, calling_ufunction, parameters);
#if defined(_DEBUG)
	indent_level--;
#endif
}
									...
									void *result)
{
	...
	PLOG_VERBOSE << GetIndentedTabString() << std::format("{0} : {1}", calling_ufunction->GetFullName(), calling_uobject->GetFullName());
#endif
	processinternal_hooks.ExecuteHook(calling_ufunction, calling_uobject, stack, result);
#if defined(_DEBUG)
	indent_level--;
#endif
}
...
UFunctionHooks<InvokePrototype> invoke_hooks(nullptr);
...
#endif
	invoke_hooks.ExecuteHook(calling_ufunction, calling_ufunction, calling_uobject, stack, result);
#if defined(_DEBUG)
	indent_level--;
#endif
}
								 ...
								 void *result, UFunction *calling_ufunction)
{
	...
	{
		return original_callfunction(calling_uobject, stack, result, calling_ufunction);
	}
...
#endif
	callfunction_hooks.ExecuteHook(calling_ufunction, calling_uobject, stack, result, calling_ufunction);
#if defined(_DEBUG)
	indent_level--;
#endif
}